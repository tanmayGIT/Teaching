...
#include "extParserpourcentage.h"
using namespace inout;

void * EXTParserClassMatchnig::readFile(void * document)
{
	bool first = true;
    FILE* doc;
    ...
    {
        exc::SequenceMatchingException::genererException("EXT Reading error : unable to open the file",ERREUR_FILE,__LINE__);
    }
    ...
    char * where;
    if((where = (char *)malloc(size*(sizeof(char)+1))) == 0)
    {
            exc::SequenceMatchingException::genererException("EXT Reading error : unable to open the file",ERREUR_FILE,__LINE__);
    }
    where[size] = '\0';
   ...
   while(strlist.at(strlist.size()-1).compare("NEXT")!=0 &&  strlist.at(strlist.size()-1).compare("\n")!=0 && strlist.size()>1)
    {
   ...
   strlist.push_back("END");
    ...
    int type= NUMERIC;//checkTexDocument(strlist);
