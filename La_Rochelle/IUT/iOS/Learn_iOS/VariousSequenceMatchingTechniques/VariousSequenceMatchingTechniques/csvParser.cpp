...
void * CSVParser::readFile(void * document)
{
    ...
    {
        exc::SequenceMatchingException::genererException("CSV Reading error : unable to open the file",ERREUR_FILE,__LINE__);
    }
    ...
    string parsed;
    stringstream input_stringstream(st);

    while(getline(input_stringstream,parsed,';'))
    {
        std::string::iterator end_pos = std::remove(parsed.begin(), parsed.end(), '\n');
        parsed.erase(end_pos, parsed.end());
        strlist.push_back(parsed);
    }
    while(strlist.at(strlist.size()-1).compare("END")!=0 && strlist.size()>1)
    {
        strlist.erase(strlist.end()-1);
    }

    int type= checkDocument(strlist);
    ...
    // root node Sequences
    rapidxml::xml_node<>* sequences_root = docr->allocate_node(rapidxml::node_element, "Sequences");
    docr->append_node(sequences_root);
    // root node type
    rapidxml::xml_node<>* type_root = docr->allocate_node(rapidxml::node_element, "Type");
    ...
    // root node sequence
    rapidxml::xml_node<>* seq_root = docr->allocate_node(rapidxml::node_element, "Sequence");
...
#else
    exc::SequenceMatchingException::genererException("Unknown platform",ERREUR_FILE,__LINE__);
#endif
    ...
    while(it<end)
    {
        if(strlist.at(it).find("NEXT")==string::npos)
        {
            ...
            {
                if(strlist.at(it).find("V")!=string::npos && strlist.at(it).find("ENDV")==string::npos)
                {
                    elt_node = docr->allocate_node(rapidxml::node_element, "Element");
            ...
            exc::SequenceMatchingException::genererException("Unknown platform",ERREUR_FILE,__LINE__);
                ...
                if(cas == 2 && ok)
                {
                    ...
                    vec_node = docr->allocate_node(rapidxml::node_element, "VectorElement");
                    ...
                    vec_node->append_attribute(docr->allocate_attribute("id", strval));
                    listPoubelle.push_back(strval);
                    elt_node->append_node(vec_node);
                    if(strlist.at(it+1).find("ENDV")!=string::npos)
                    {
                        cas = 3;
                    }
                    else
